...
Very basic script to transform a pkl into .vtp for 3D visualization in software like ParaView.
...
2. ATTRIBUTE TRANSFER (Cell Data):
   ...
   - These attributes are stored as 'Cell Data', meaning each line segment 
     carries its own physical and classification properties (the contrary will be PointData, info per point = tortuous).
   - 'nkind' is set as the active scalar for color-coding in ParaView.

3. EXPORT:
   With vtkXMLPolyDataWriter we create the .vtp
