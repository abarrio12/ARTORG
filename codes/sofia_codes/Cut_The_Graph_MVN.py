...
def get_edges_in_boundingBox_vertex_based(xCoordsBox=[1300,1700],yCoordsBox=[1400,1800],zCoordsBox=[0, 400]):
    ...
    # important for edge order  (who is origin and who target)
    for e in G.es:
        points = e['points']
        if G.vs[e.source]['coords'].all() != points[0].all(): # e = (u,v) -> checks ux,uy,uz != x1,y1,z1 of the points IMPORTANT: points includes node points + internal points
            node_0 = e.target
            node_1 = e.source
        else:
            node_0 = e.source
        ...
        vertices_in_box = [0, 0] # initialize with 0,0 but will be filled with 1/0 depending if vertex is in or out
        ...
        if np.sum(vertices_in_box) == 2: #both vertices inside the box
            edges_in_box.append(e.index)
            
        elif np.sum(vertices_in_box) == 1: #one vertex inside the box, one outside (1,0) o (0,1)
            ...
            # Groups sequences as in/out through the edge points
            # consecutive = [0, 0, 0, 1, 1, 1, 1] -> 3 first out, 4 rest in
            # frequency = [3, 4]
            frequency = []
            count = 1
            ...
            # external points deleted and internal points kept (only points inside box are interesting)
            ...
            for axis, (min_val, max_val) in enumerate(zip([xCoordsBox[0], yCoordsBox[0], zCoordsBox[0]], [xCoordsBox[1], yCoordsBox[1], zCoordsBox[1]])):
                if external_point[axis] < min_val or external_point[axis] > max_val:
                    ...
                    if node_index is None:
                        ...
                        # [-1] refers to last item on the list
...
# --------------------------------------------------------------------------------------------------------------------------
# THIS IS USEFUL IF THE GRAPH IS NOT YET CREATED (creation from dictionaries of edges and vertices).
# IF THE GRAPH IS ALREADY CREATED, SKIP TO THE NEXT BLOCK (ANA's comment)
...
# with open("vertices_Artemisia_Entire_MVN.pkl.pkl", 'rb') as file:
#     # Load data from the .pkl file using the appropriate protocol
...
# # IF THE GRAPH IS ALREADY CREATED, RUN THIS BLOCK (ANA's comment)

with open(r"C:/Users/Ana/OneDrive/Escritorio/ARTORG/igraph/MVN1 scripts/MVN1_corrected_SI.pkl", 'rb') as f:
    G = pickle.load(f, encoding='latin1')
    
print("puntos", G.es["points"][:1]) # points is a list of points. Each edge has 3 points [apuntos [array([x1, y1, z1]), array([x2, y2, z2]), array([x3, y3, z3])]
# --------------------------------------------------------------------------------------------------------------------------
...
edges_in_box, edges_across_border, edges_outside_box, border_vertices, new_edges_on_border = get_edges_in_boundingBox_vertex_based(xCoordsBox=[0.0010, 0.0014], yCoordsBox=[0.0016, 0.0020], zCoordsBox=[0.0004, 0.0008])
...
# --------------------------------------------------------------------------------------------------------------------------
# If you want to save the new subgrafh as a dictionary after cutting it with the bounding box, run this block (ANA's comment)
# --------------------------------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------------------------------
# Save entire subgraph to pickle file
with open('MVN1_CUT.pkl', 'wb') as f:
    pickle.dump(G, f)
# --------------------------------------------------------------------------------------------------------------------------
